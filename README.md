...
| 다국어 | next-intl i18n | 한국어와 영어를 기본 제공하며, locale과 번역 파일을 추가해 다른 언어로 확장할 수 있습니다. |
...
### 4. 다국어: next-intl i18n 기반 확장

기본 언어는 한국어이며, 영어를 함께 제공합니다.

- `/ko`와 `/en` 라우팅을 제공합니다.
- 상단 언어 선택은 드롭다운입니다.
- 한국어를 선택하면 한국어 페이지로 이동합니다.
- English를 선택하면 영어 페이지로 이동합니다.
- 일본어, 중국어, 스페인어 등 새 언어는 `i18n/routing.ts`, `lib/site.ts`, `messages/{locale}.json`, locale별 카피와 콘텐츠를 추가해 확장할 수 있습니다.
- 아직 번역 파일이 없는 언어는 English를 폴백 언어로 사용할 수 있도록 설계합니다.
